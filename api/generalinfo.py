...
import secrets
from models import UpdatePassword,SignaturesByName
...
@info_signature_route.get("/get_api_token")
async def get_api_token(name: SignaturesByName, current_user: dict = Depends(get_current_user)):
    if not current_user:
        raise HTTPException(status_code=401, detail="No autorizado, no existe el usuario activo")

    user_id = current_user["id"]
    api_token = get_api_token_by_name_and_user_id(name.signature_name, user_id)
...
@info_signature_route.put("/update_api_token")
async def update_api_token(name: SignaturesByName, current_user: dict = Depends(get_current_user)):
    if not current_user:
        raise HTTPException(status_code=401, detail="No autorizado, no existe el usuario activo")

    user_id = current_user["id"]
    api_key = secrets.token_hex(32)
    updated_rows, new_api_key = update_api_token_by_name_and_user_id(name.signature_name, api_key, user_id)
