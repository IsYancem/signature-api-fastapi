...
class XAdESContext(SignatureContext):
    ...
    def calculate_signed_properties(self, signed_properties, node, sign=False):
        ...
        if data_object_properties is not None:
            self.calculate_data_object_properties(data_object_properties, node, sign)
        return

    def calculate_signature_properties(self, signature_properties, node, sign=False):
        signing_time = signature_properties.find("etsi:SigningTime", namespaces=NS_MAP)
