...
from .constants import ID_ATTR, NS_MAP, EtsiNS
...
def create_qualifying_properties(node, name=None, etsi="etsi"):
    ...
    qualifying = etree.SubElement(
        obj_node, etree.QName(EtsiNS, "QualifyingProperties"), nsmap={etsi: EtsiNS}
    )
...
def create_signed_properties(node, name=None, datetime=None):
    ...
    signature_properties = create_node("SignedSignatureProperties", properties, EtsiNS)
...
def add_production_place(node, city=None, state=None, postal_code=None, country=None):
...
def add_claimed_role(node, role):
    ...
    signer_role = signature_properties.find("etsi:SignerRole", namespaces=NS_MAP)
    if signer_role is None:
        signer_role = create_node("SignerRole", signature_properties, ns=EtsiNS)
...
def ensure_signed_data_object_properties(node):
    properties = node.find("etsi:SignedDataObjectProperties", namespaces=NS_MAP)
    if properties:
        return properties
...
def add_data_object_format(
    node, reference, description=None, identifier=None, mime_type=None, encoding=None
):
    data_object_node = create_node("DataObjectFormat", ns=EtsiNS)
    node.insert(
        len(node.findall("etsi:DataObjectFormat", namespaces=NS_MAP)), data_object_node
    )
    data_object_node.set("ObjectReference", reference)
    if description is not None:
        create_node("Description", data_object_node, EtsiNS).text = description
    if identifier is not None:
        identifier.to_xml(create_node("ObjectIdentifier", data_object_node, EtsiNS))
...
def add_commitment_type_indication(
    ...
    if references is None:
        ...
        for reference in references:
            create_node("ObjectReference", commitment_type, EtsiNS).text = reference
    if qualifiers_type is not None:
        qualifiers = create_node("CommitmentTypeQualifiers", commitment_type, EtsiNS)
        for qualifier in qualifiers_type:
            create_node("CommitmentTypeQualifier", qualifiers, EtsiNS).text = qualifier
